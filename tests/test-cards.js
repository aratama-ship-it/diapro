...
test('retired: 無傷の四年間・文武両道は早期引退では取れない（4年前提カード）', () => {
  // 4位×10大会(表彰台なし)で850pt=Aランク・怪我ゼロ・学力95: 卒業ならsp_unhurtが最上位
  const mk = s => {
    s.injuryCount = 0; s.study = 95;
    s.results = Array.from({ length: 10 }, (_, i) => res({ rank: 4, points: 85, turn: i + 2 }));
    return s;
  };
  const grad = mk(base());
  assert.strictEqual(DT.cards.pickCard(grad).id, 'sp_unhurt');
  assert.ok(DT.cards.pickCandidates(grad).some(c => c.id === 'sp_scholar'), '卒業なら文武両道も候補');
  const ret = mk(base({ status: 'retired' }));
  const cands = DT.cards.pickCandidates(ret);
  assert.ok(!cands.some(c => c.id === 'sp_unhurt'), '引退では無傷の四年間を除外');
  assert.ok(!cands.some(c => c.id === 'sp_scholar'), '引退では文武両道を除外');
  assert.strictEqual(DT.cards.pickCard(ret).layer, 'matrix'); // 受け皿に落ちる
  assert.strictEqual(DT.cards.pickCard(ret).rank, 'A');
});

test('retired: 引退時点で達成済みの実績カード(日本の頂点等)は有効', () => {
  const s = base({ status: 'retired' });
  s.results = [res({ type: 'ajdc', rank: 1, points: 100, turn: 24 })];
  assert.strictEqual(DT.cards.pickCard(s).id, 'sp_ajdc'); // AJDC総合優勝→S判定も維持
  assert.strictEqual(DT.cards.pickCard(s).rank, 'S');
});
