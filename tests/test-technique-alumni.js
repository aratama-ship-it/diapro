...
test('初期卒業生3名の名前と得意技が決定どおり定義される', () => {
  assert.deepStrictEqual(
    DT.DATA.DEFAULT_ALUMNI.map(a => [a.name, a.techniqueId]),
    [
      ['工藤まさし', 'high_toss'],
      ['綿貫しゅうすけ', 'fts'],
      ['深田あきら', 'on_beat']
...
test('卒業生名簿は2〜5名だけ保存できる', () => {
  ...
  assert.strictEqual(DT.state.saveAlumniSelection(ids.slice(0, 2), store, 'short').ok, true);
  assert.deepStrictEqual(DT.state.loadAlumniProfile(store, 'short').selectedIds, ids.slice(0, 2));
  assert.strictEqual(DT.state.saveAlumniSelection(ids.concat(['unknown_1', 'unknown_2', 'unknown_3']), store, 'short').ok, true);
...
test('選んだ卒業生だけが次周回の先輩抽選に使われる', () => {
  ...
  graduate.name = '育成した先輩';
  const added = DT.state.addGraduateAlumni(
    graduate,
    { rank: 'A', type: 'power', title: '剛技の使い手', totalPoints: 850, cp: 780 },
    store,
    'short',
    6000
  );
  const selected = [added.alumni.id, 'fukada_akira'];
  ...
  assert.deepStrictEqual(schedule, [
    { turn: 28, alumniId: added.alumni.id },
    { turn: 38, alumniId: 'fukada_akira' }
  ]);
});
...
test('卒業生イベントは3年次・4年次に各1回、別の先輩で予定される', () => {
  ...
  assert.deepStrictEqual(schedule, [
    ...
    { turn: 38, alumniId: 'watanuki_shusuke' }
  ]);
  assert.notStrictEqual(schedule[0].alumniId, schedule[1].alumniId);
});
...
test('練習方法の成功は全新奇性+1・構成+1、失敗はやる気-5', () => {
  ...
  DT.events.applyAlumniChoice(fail, failEv, 2, () => 0.9);
...
test('ハプニングは大会の審査ぶれを±3から±8へ広げる', () => {
  ...
  assert.strictEqual(DT.contest.playerScore(happening, 'overall', judgeLow()).judgeMod, -8);
});
