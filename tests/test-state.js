...
test('newCharacter: 経歴で初期能力レンジが変わる（skills/composition共通・学力は共通）', () => {
  const hard = DT.state.newCharacter(() => 0, 'college');
  DT.DATA.GENRES.forEach(g => DT.DATA.METHODS.forEach(m => assert.strictEqual(hard.skills[g.id][m.id], 5)));
  assert.strictEqual(hard.composition, 5);
  ...
  assert.strictEqual(easyMax.composition, 55);
