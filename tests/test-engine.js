...
test('computeSlotGain経由: motivation80以上（絶好調）で成功枠ゲインに+1、失敗枠には乗らない', () => {
  ...
  assert.strictEqual(r.results[0].gain, 4); // round(2*1*1*1.5[1年目])=3 +hotBonus(1) = 4
  assert.strictEqual(s.skills.v1d.difficulty, 14);
  ...
  assert.strictEqual(rBelow.results[0].gain, 3); // hotLine未満なのでボーナスなし（1年目1.5倍のみ）
});
...
// 各枠: gain=round(2×1.0×growthMult(現在値)×1.5[1年目])=3。growthMultは10→13→16→19はいずれも<40なので常に1.0。
// → skills.h1d.control = 10 + 3×4 = 22
...
test('applyTraining: 検算ピン 全枠{h1d,control}・rng0.3固定', () => {
  ...
  assert.strictEqual(s.skills.h1d.control, 22);
...
test('applyTraining: 基本ゲイン（単枠・成功、マス以外のジャンル/methodは不変）', () => {
  ...
  assert.strictEqual(s.skills.h1d.control, 13); // 10 + round(2*1*1*1.5[1年目])=3
  ...
  assert.ok(r.messages[0].includes('操作安定度 +3') || r.messages[0].includes('+3'));
});
...
test('applyTraining: routine枠はcompositionのみ加算・skillsは不変', () => {
  ...
  assert.strictEqual(s.composition, 12); // 10 + round(1*1*1*1.5[1年目])=round(1.5)=2
  ...
  assert.ok(r.messages[0].includes('演技構成 +2'));
});
...
test('applyTraining: 大会月のroutineはブースト、difficultyはペナルティ+追加疲労、controlは倍化', () => {
  ...
  assert.strictEqual(sR.composition, 12); // round(1×1×1×1.5[大会routine]×1.5[1年目])=round(2.25)=2 → 10+2
  const sD = mk(5);
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(sD.skills.v1d.difficulty, 12); // round(2×1×1×0.5[大会difficulty]×1.5[1年目])=round(1.5)=2 → 10+2
  ...
  assert.strictEqual(sC.skills.v1d.control, 16); // round(2×1×1×2.0[大会control]×1.5[1年目])=6 → 10+6
});
...
test('applyTraining: 練習会月はroutine/noveltyの伸びが1.5倍、他は不変', () => {
  ...
  DT.engine.applyTraining(sR, ['routine'], () => 0.3);
  assert.strictEqual(sR.composition, 12); // round(1×1×1×1.5[練習会]×1.5[1年目])=round(2.25)=2 → 10+2
  const sN = mk(3);
  DT.engine.applyTraining(sN, [{ genre: 'v1d', method: 'novelty' }], () => 0.3);
  assert.strictEqual(sN.skills.v1d.novelty, 15); // round(2×1×1×1.5[練習会]×1.5[1年目])=round(4.5)=5 → 10+5
  const sD = mk(3);
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(sD.skills.v1d.difficulty, 13); // 練習会対象外: round(2×1×1×1.5[1年目])=3 → 10+3
});
...
test('applyTraining: 特別指導解放で成功枠ごとに+1（タイミング補正の後・フラット加算）', () => {
  ...
  DT.engine.applyTraining(s, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(s.skills.v1d.difficulty, 14); // round(2×1×1×1.5[1年目])=3 +1 = 4 → 10+4
  const sFail = base();
  ...
  assert.strictEqual(sMeetup.skills.v1d.novelty, 16); // round(2×1.5[練習会]×1.5[1年目])=round(4.5)=5 +1 = 6 → 10+6
});
...
test('applyTraining: routineスロットのメッセージは「ルーチン構成（成功）: 演技構成 +N」形式', () => {
  const s = base();
  const r = DT.engine.applyTraining(s, ['routine'], () => 0.3);
  assert.strictEqual(r.messages[0], 'ルーチン構成（成功）: 演技構成 +2'); // 1年目1.5倍でgain2
});
