...
// → difficulty 10+1×4=14, genres.v1d 10+1×4=14, fatigue 5×4=20, injuryRisk 10+4×3=22
test('applyTraining: 検算基準 全枠{v1d,difficulty}・rng0.3固定', () => {
  ...
  assert.strictEqual(s.injuryRisk, 22);
...
test('applyTraining: 失敗枠はゲインゼロ・疲労とリスクは通常通り加算', () => {
  ...
  assert.strictEqual(s.injuryRisk, 11); // 10 + SLOTS.risk.novelty(1)
