...
// → difficulty 10+3×4=22, genres.v1d 10+2×4=18, fatigue 5×4=20, injuryRisk 10+2×4=18
test('applyTraining: 検算基準 全枠{v1d,difficulty}・rng0.3固定', () => {
  ...
  assert.strictEqual(s.stats.difficulty, 22);
  assert.strictEqual(s.genres.v1d, 18);
  ...
  r.results.forEach(res => {
    ...
    assert.strictEqual(res.methodGain, 3);
    assert.strictEqual(res.genreGain, 2);
  });
});
...
test('applyTraining: 基本ゲイン（単枠・成功）', () => {
  ...
  assert.strictEqual(s.stats.control, 13); // 10 + round(3*1*1)
  assert.strictEqual(s.genres.h1d, 12); // 10 + round(2*1*1)
  ...
  assert.ok(r.messages[0].includes('操作安定度 +3'));
  assert.ok(r.messages[0].includes('習熟 +2'));
});
...
test('applyTraining: routine枠はcomposition+のみ・ジャンル不変', () => {
  ...
  assert.strictEqual(s.stats.composition, 13); // 10 + round(3*1*1)
...
test('applyTraining: 大会月のroutineはブースト、difficultyはペナルティ+追加疲労、controlは倍化', () => {
  ...
  assert.strictEqual(sR.stats.composition, 15); // round(3*1*1)=3 → ×1.5 → round(4.5)=5 → 10+5
  const sD = mk(5);
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(sD.stats.difficulty, 12); // round(3*1*1)=3 → ×0.5 → round(1.5)=2 → 10+2
  assert.strictEqual(sD.genres.v1d, 12); // genreGainはタイミング補正なし: 10+2
  ...
  assert.strictEqual(sC.stats.control, 16); // round(3*1*1)=3 → ×2 → 6 → 10+6
});
...
test('applyTraining: 練習会月はroutine/noveltyの伸びが1.5倍、他は不変', () => {
  const sR = mk(3); // 練習会月
  DT.engine.applyTraining(sR, ['routine'], () => 0.3);
  assert.strictEqual(sR.stats.composition, 15); // 3 → ×1.5 → round(4.5)=5 → 10+5
  const sN = mk(3);
  DT.engine.applyTraining(sN, [{ genre: 'v1d', method: 'novelty' }], () => 0.3);
  assert.strictEqual(sN.stats.novelty, 15); // 3 → ×1.5 → 5 → 10+5
  assert.strictEqual(sN.genres.v1d, 12); // genreGainは補正なし
  const sD = mk(3);
  DT.engine.applyTraining(sD, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(sD.stats.difficulty, 13); // 対象外: round(3*1*1)=3のまま
});
...
test('applyTraining: 特別指導解放で成功枠ごとに+1（タイミング補正の後・フラット加算）', () => {
  ...
  DT.engine.applyTraining(s, [{ genre: 'v1d', method: 'difficulty' }], () => 0.3);
  assert.strictEqual(s.stats.difficulty, 14); // round(3*1*1)=3 +1 = 4 → 10+4
  const sFail = base();
  ...
  assert.strictEqual(sMeetup.stats.novelty, 16); // round(3*1.5)=5 +1 = 6 → 10+6
});
