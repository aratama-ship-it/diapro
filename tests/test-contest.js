...
test('runDivision: 対戦相手の生成値も同一リマップ36（oidc1年・mean35・ノイズ0）', () => {
  const s = allFifty();
  const rs = DT.contest.runAll(s, DT.DATA.CONTESTS[0], ['overall'], () => 0.5); // 1年OIDC 相手平均35（v5バランス調整）
  // 相手の生値=mean(ノイズ0)=35 → display 36+35*0.7=60.5。全員プレイヤー81.5未満→rank1
  assert.strictEqual(rs[0].rank, 1);
  // standings[0]=自分(81.5) standings[1..2]=モブ上位(60.5、同点) standings[3]=志音(ライバル,51.4、モブより弱く最下位16位)
  assert.strictEqual(rs[0].standings[1].score, 60.5);
  assert.strictEqual(rs[0].standings[2].score, 60.5); // モブの生成値=mean35→display60.5（複数モブが同点）
  const shionRow = rs[0].standings.find(e => e.name === '志音');
  assert.strictEqual(shionRow.score, 51.4);
  assert.strictEqual(shionRow.rank, 16); // 志音51.4はモブ60.5群より弱く最下位
});
